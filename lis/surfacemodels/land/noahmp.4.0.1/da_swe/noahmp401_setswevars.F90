  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(noahmp401_struc(n)%noahmp401(t)%sneqv.gt.0) then
        ...
        if(abs(dsnowh).ge.0.010) then           
           ! update
           if(t.eq.21.and.dsneqv.lt.0) then
              print*, 'incr ',dsneqv,dsnowh
              print*, noahmp401_struc(n)%noahmp401(t)%sneqv,noahmp401_struc(n)%noahmp401(t)%snowh,&
                   noahmp401_struc(n)%noahmp401(t)%runsf
           endif
           call noahmp401_snow_update(n, t, dsneqv, dsnowh)
           if(t.eq.21.and.dsneqv.lt.0) then
              print*, 'af ',noahmp401_struc(n)%noahmp401(t)%sneqv,noahmp401_struc(n)%noahmp401(t)%snowh,&
                   noahmp401_struc(n)%noahmp401(t)%runsf
           endif
        endif
     endif
